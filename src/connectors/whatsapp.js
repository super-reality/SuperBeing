...
import { getRandomEmptyResponse, startsWithCapital, getSetting } from './utils.js';
...
export class whatsapp_client {
    ...
    async onMessage(msg) {
        ...
        if (msg.chat.type == 'supergroup') {
            ...
            if (!msg.from.is_bot) {
                ...
                if (this.prevMessageTimers[msg.chat.id] !== undefined) clearTimeout(this.prevMessageTimers[msg.chat.id])
                this.prevMessageTimers[msg.chat.id] = setTimeout(() => this.prevMessage[msg.chat.id] = '', 120000)
            }
            addPing = _prev !== undefined && _prev !== '' && _prev !== _sender

            const isMention = msg.entities !== undefined && msg.entities.length === 1 && msg.entities[0].type === 'mention' && content.includes('@' + botName)
            const otherMention = msg.entities !== undefined && msg.entities.length > 0 && msg.entities[0].type === 'mention'  && !content.includes('@' + botName)
            let startConv = false
            let startConvName = ''
            ...
            if (!otherMention && content.startsWith('!ping')) this.sentMessage(_sender)
        }
        else {
            ...
            Object.keys(response.response).map(function(key, index) {
                ...
                if (response.response[key] !== undefined && response.response[key].length <= 2000 && response.response[key].length > 0) {
                    ...
                    if (addPing) msg.reply(msg.chat.id,`<a href="tg://user?id=${msg.from.id}">${msg.from.first_name}</a> ${text}`, {parse_mode: 'HTML'}).then(function (_resp) {
                        this.onMessageResponseUpdated(_resp.chat.id, msg.message_id, _resp.message_id)
                        this. addMessageToHistory(_resp.chat.id, _resp.message_id, botName, text)
                        }).catch(console.error)
                    ...
                    if (addPing) msg.reply(msg.chat.id,`<a href="tg://user?id=${msg.from.id}">${msg.from.first_name}</a> ${emptyResponse}`, {parse_mode: 'HTML'}).then(function (_resp) {
                        this.onMessageResponseUpdated(_resp.chat.id, msg.message_id, _resp.message_id)
                        this.addMessageToHistory(_resp.chat.id, _resp.message_id, botName, emptyResponse)
                        }).catch(console.error)         
    ...
    sentMessage(user) {
        ...
        this.conversation[user] = setTimeout(function() { this.conversation[user] = undefined }, 120000)
    }
