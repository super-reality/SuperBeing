...
import { getRandomEmptyResponse, getRandomTopic, getSetting, startsWithCapital } from "./utils.js"
...
export class discord_client {
    ...
    async pingagent (client, message, args, author, addPing, channel) {
        ...
        || args.grpc_args.message.includes('agent=') || args.grpc_args.agent === undefined || args.grpc_args.agent === '' || args.grpc_args.agent.replace(/\s/g, '').length === 0) {
            this.client.embed.description = 'Wrong format, !pingagent agent=agent message=value'
            message.channel.send(client.embed)
            this.client.embed.desscription = ''
            message.channel.stopTyping();
            return
    ...
    async setagent (client, message, args, author, addPing, channel) {
        if (args.grpc_args.message === undefined || args.grpc_args.message === '') {
            this.client.embed.description = 'Wrong format, !setagent agent=agent context=value'
            message.channel.send(client.embed)
            this.client.embed.desscription = ''
            message.channel.stopTyping();
            return
    ...
    messageCreate = async (client, message) => {
            ...
            for (let i = 0; i < data.length; i++) {
                if (data[i].startsWith('<@!') && data[i].charAt(data[i].length - 1) === '>') {
                    try {
                        ...
                        const user = await this.client.users.cache.find(user => user.id == x)
                        if (user !== undefined) {
                            //const u = '@' + user.username + '#' + user.discriminator
                            const u = user.id == this.client.user ? this.agent.name : user.username
        ...
        if (!author.bot) {
            ...
            if (this.prevMessageTimers[channel.id] !== undefined) clearTimeout(pthis.revMessageTimers[channel.id])
            this.prevMessageTimers[channel.id] = setTimeout(() => this.prevMessage[channel.id] = '', 120000)
        }
        //if there are many users in the conversation simulation or the previous message is from someone else, it adds a ping
        const addPing = (_prev !== undefined && _prev !== '' && _prev !== author) || this.moreThanOneInConversation()
        // Ignore all bots
        ...
        if (otherMention) {
            ...
            if (this.discussionChannels[channel.id] !== undefined && this.discussionChannels[channel.id]) {
        ...
        const prefixOptionalWhenMentionOrDM = this.client.config.prefixOptionalWhenMentionOrDM
        ...
        const containsPrefix = messageContent.indexOf(this.client.config.prefix) === 0;
        ...
        const response = await handleInput(message.content, message.author.username, this.agent.name ?? "Agent", null, 'discord', channel.id);
        this.messageEvent.emit('new_message', this, response, addPing, channel, message);
    };
    ...
    messageDelete = async (client, message) => {
        ...
        await channel.messages.fetch({ limit: this.client.edit_messages_max_count }).then(async messages => {
    ...
    ready = async (client) => {
        await this.client.users.fetch(customConfig.instance.get('logDMUserID')).then((user) => {
        ...
        await this.client.guilds.cache.forEach((server) => {
            if (!server.deleted) {
                ...
                this.client.api.applications(client.user.id).guilds(server.id).commands.post({
                    data: {
                        name: "continue",
                    ...
                    if (channel.type === channelTypes['text'] && channel.deleted === false && channel.permissionsFor(client.user.id).has(['SEND_MESSAGES', 'VIEW_CHANNEL'])) {
                        ...
                        channel.messages.fetch({ limit: 100 }).then(async messages => {
                            messages.forEach(async function (msg) {
                                ...
                                if (msg.author.isBot || msg.author.username.toLowerCase().includes('digital being')) _author = this.agent.name
    ...
    onMessageDeleted(channel, messageId) {
        if (this.messageResponses[channel] !== undefined && this.messageResponses[channel][messageId] !== undefined) {
            delete this.messageResponses[channel][messageId]
        }
    }
    onMessageResponseUpdated(channel, messageId, newResponse) {
        if (this.messageResponses[channel] === undefined) this.messageResponses[channel] = {}
    ...
    async addMessageInHistoryWithDate(chatId, messageId, senderName, content, timestamp) {
        if (!database || !database.instance) return // log("Postgres not inited");
    ...
    createDiscordClient = (agent, settings) => {
        ...
        this.client.username_regex = new RegExp(customConfig.instance.get('botNameRegex'), 'ig')
        this.client.edit_messages_max_count = customConfig.instance.getInt('editMessageMaxCount')
        ...
        this.client.on("presenceUpdate", this.presenceUpdate.bind(null, this.client));

        this.messageEvent = new EventEmitter();
        this.messageEvent.on('new_message', async function(ds, responses, addPing, channel, message) { 
            log('response: ' + responses)
            if (responses !== undefined && responses.length <= 2000 && responses.length > 0) {
        ...
        setInterval(() => {
            ...
            this.client.channels.cache.forEach(async (channel) => {
                ...
                if (discussionChannels[channel.id] === undefined || !discussionChannels) {
                    ...
                    responded: false };
                    const resp = await handleInput('Tell me about ' + getRandomTopic(), 'bot', this.agent.name ?? "Agent", null, 'discord', channel.id);
                    channel.send(resp);
                }
