...
import { getRandomEmptyResponse, getSetting, startsWithCapital } from "./utils.js"
...
export class telegram_client {
        ...
        if (args !== 'none' && args.startsWith('[') && args[args.length-1] === ']') {
            ...
            if (response !== undefined && response.length > 0) {
                ...
                if (addPing) bot.sendMessage(chat_id,`<a href="tg://user?id=${senderId}">${senderName}</a> ${text}`, {parse_mode: 'HTML'}).then(function (_resp) {
                    this.onMessageResponseUpdated(_resp.chat.id, message_id, _resp.message_id)
                    this.addMessageToHistory(_resp.chat.id, _resp.message_id, this.agent.name, text)
                    }).catch(console.error)
                ...
                if (addPing) bot.sendMessage(chat_id,`<a href="tg://user?id=${senderId}">${senderName}</a> ${emptyResponse}`, {parse_mode: 'HTML'}).then(function (_resp) {
                    this.onMessageResponseUpdated(_resp.chat.id, message_id, _resp.message_id)
                    this.addMessageToHistory(_resp.chat.id, _resp.message_id, this.agent.name, emptyResponse)
                }).catch(console.error)           
            ...
            if (args !== 'none' && args.startsWith('[') && args[args.length-1] === ']') {
                ...
                if (response !== undefined && response.length <= 2000 && response.length > 0) {
                    ...
                    bot.sendMessage(chat_id,`<a href="tg://user?id=${senderId}">${senderName}</a> ${text}`, {parse_mode: 'HTML'}).then(function (_resp) {
                        this.onMessageResponseUpdated(_resp.chat.id, message_id, _resp.message_id)
                        this.addMessageToHistory(_resp.chat.id, _resp.message_id, telegramPacketHandler.instance.botName, text)
                        }).catch(console.error)
                }
                    ...
                    bot.sendMessage(chat_id,`<a href="tg://user?id=${senderId}">${senderName}</a> ${emptyResponse}`, {parse_mode: 'HTML'}).then(function (_resp) {
                        this.onMessageResponseUpdated(_resp.chat.id, message_id, _resp.message_id)
                        this.addMessageToHistory(_resp.chat.id, _resp.message_id, telegramPacketHandler.instance.botName, emptyResponse)
                    }).catch(console.error)   
                }
        }
    ...
    async onMessageEdit(bot, msg, botName) {
        ...
        await this.handleEditMessage(msg.chat.id, msg.message_id, resp, '[ \''+ msg.from.id + '\', \'' + msg.from.first_name + '\' ]', bot);
    }

    async onMessage(bot, msg, botName, username_regex) {
        this.addMessageToHistory(msg.chat.id, msg.message_id, msg.from.username === undefined ? msg.from.first_name : msg.from.username, msg.text)
        ...
        if (msg.chat.type == 'supergroup') {
            ...
            if (!msg.from.is_bot) {
                ...
                this.prevMessageTimers[msg.chat.id] = setTimeout(() => prevMessage[msg.chat.id] = '', 120000)
            }
            addPing = (_prev !== undefined && _prev !== '' && _prev !== _sender) || this.moreThanOneInConversation()

            const isMention = msg.entities !== undefined && msg.entities.length === 1 && msg.entities[0].type === 'mention' && content.includes('@' + getSetting(this.settings, 'telegramBotName'))
            const otherMention = msg.entities !== undefined && msg.entities.length > 0 && msg.entities[0].type === 'mention'  && !content.includes('@' + getSetting(this.settings, 'telegramBotName'))
            let startConv = false
            let startConvName = ''
        ...
        const resp = handleInput(msg.text, msg.from.first_name, customConfig.instance.get('agent') ?? "Agent", null, 'telegram', msg.chat.id);
        await this.handleEditMessage(msg.chat.id, msg.message_id, resp,addPing ? '[ \''+ msg.from.id + '\', \'' + msg.from.first_name + '\' ]' : 'none', bot);
    }
