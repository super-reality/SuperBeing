  ...
  async handleMessage(senderPsid, receivedMessage) {
    ...
    if (receivedMessage.text) {
      await database.instance.getNewMessageId('messenger', senderPsid, async (msgId) => {
        this.addMessageToHistory(senderPsid, senderPsid, receivedMessage.text, msgId)
        const message = receivedMessage.text
        const resp = await handleInput(message, senderPsid, this.agent.name ?? "Agent", null, 'messenger', senderPsid);
