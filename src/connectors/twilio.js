...
import { getRandomEmptyResponse, getSetting } from './utils.js';
...
export class twilio_client {
    async message(req, res) {
        ...
        await database.instance.getNewMessageId('twilio', req.body.From, async (msgId) => {
            this.addMessageToHistory(req.body.From, req.body.From, req.body.Body, msgId)   
            ...
            const resp = await handleInput(req.body.Body, req.body.From, this.agent.name ?? "Agent", null, 'twilio', req.body.From);
    ...
    sendMessage(client, toNumber, body) {
        ...
        client.messages.create({from: getSetting(this.settings, 'twilioPhoneNumber'),
            to: toNumber,
            body: body
