...
export class twitter_client {
    async handleMessage(response, chat_id, args, twitter, twitterV1, localUser) {
        if (args === 'DM') {
            ...
            database.instance.addMessageInHistory('twitter', chat_id, dmSent.event.id, this.agent.name, response)
        } else if (args === 'Twit') {
            await twitterV1.v1.reply(response, chat_id).then(res => {
                database.instance.addMessageInHistory('twitter', chat_id, res.id_str, this.agent.name, response)
            })
        }
    }
    ...
    createTwitterClient = async (agent, settings) => {
        ...
        const twitterAccessTokenSecret = getSetting(settings, 'twitterAccessTokenSecret');
        ...
        setInterval(async () => {
            ...
            for await (const event of eventsPaginator) {
                ...
                if (event.type == 'message_create') {
                    ...
                    await database.instance.messageExistsAsyncWitHCallback2('twitter', event.message_create.target.recipient_id, event.id, authorName, event.message_create.message_data.text, parseInt(event.created_timestamp), async () => {
                        const resp = await handleInput(event.message_create.message_data.text, authorName, this.agent.name ?? "Agent", null, 'twitter', event.id);
